                 ...
                 GtkWidget **ppwTabArgs)
{
    ...
    gtk_table_attach_defaults(GTK_TABLE(*(ppwTabArgs + 3)), *(ppwTabArgs + 1),
                              0, 1, 1, 2);
                     ...
                     GtkWidget **ppwTabArgs)
{
    ...
    *(ppwTabArgs + 5) = gtk_image_new_from_pixbuf(pGdkPixbufImaRes);
    gtk_table_attach_defaults(GTK_TABLE(*(ppwTabArgs + 4)), *(ppwTabArgs + 5),
                              0, 1, 1, 2);
    gtk_widget_show_all(*(ppwTabArgs + 2));
}
...
int main(int argc, char **argv)
{
    ...
    gtk_window_set_title(GTK_WINDOW(pwMainWind), "ImaProjet");
    ...
    gtk_table_attach_defaults(GTK_TABLE(pwTabShowOrig), pwImaOrig, 0, 1, 1, 2);

    /*la boite contenant les boutons ouverture*/
    pwButtonBoxOpen = gtk_hbutton_box_new();
    gtk_table_attach_defaults(GTK_TABLE(pwTabShowOrig), pwButtonBoxOpen, 0, 1,
                              0, 1);
    /*le bouton ouverture*/
    ...
    gtk_signal_connect(GTK_OBJECT(pwOpenFileSelection), "delete_event",
                       GTK_SIGNAL_FUNC(gtk_widget_hide), pwOpenFileSelection);
    gtk_signal_connect(GTK_OBJECT(pwOpenFileSelection), "destroy_event",
                       GTK_SIGNAL_FUNC(gtk_widget_hide), pwOpenFileSelection);
    gtk_signal_connect_object(GTK_OBJECT(pwButtonOpen), "clicked",
                              GTK_SIGNAL_FUNC(gtk_widget_show),
                              GTK_OBJECT(pwOpenFileSelection));
    gtk_signal_connect_object(
        GTK_OBJECT(GTK_FILE_SELECTION(pwOpenFileSelection)->cancel_button),
    ...
    gtk_signal_connect(
        GTK_OBJECT(GTK_FILE_SELECTION(pwOpenFileSelection)->ok_button),
        "clicked", GTK_SIGNAL_FUNC(ShowImaOrig), (gpointer)ppwTabArgs);
    gtk_signal_connect_object(
        GTK_OBJECT(GTK_FILE_SELECTION(pwOpenFileSelection)->ok_button),
        "clicked", GTK_SIGNAL_FUNC(gtk_widget_hide),
        GTK_OBJECT(pwOpenFileSelection));
    ...
    gtk_table_attach_defaults(GTK_TABLE(pwTabShowRes), pwImaRes, 0, 1, 1, 2);
    /*case 5: le GtkImage resultat*/
    ppwTabArgs[5] = pwImaRes;

    /*la boite contenant le bouton quitter*/
    pwButtonBoxRes = gtk_hbutton_box_new();
    gtk_table_attach_defaults(GTK_TABLE(pwTabShowRes), pwButtonBoxRes, 0, 1, 0,
                              1);
    ...
    gtk_signal_connect(GTK_OBJECT(pwButtonAnalyse), "clicked",
                       GTK_SIGNAL_FUNC(ShowImaComputed), ppwTabArgs);
